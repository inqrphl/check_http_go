...
func TestCertificateCheckWarn3Days(t *testing.T) {
	...
	if !strings.Contains(output.String(), "expire=") {
		t.Errorf("expected perfdata to contain 'expire='")
	}
}
...
func TestCertificateChainPerfdata(t *testing.T) {
	...
	if !strings.Contains(output.String(), "expire_chain_1=") {
		t.Errorf("expected perfdata to contain 'expire_chain_1=' for chain cert")
	}
}
